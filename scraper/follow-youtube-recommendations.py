...
class YoutubeFollower():
    ...
    def get_search_results(self, search_terms, max_results, top_rated=False):
        ...
        url = "https://www.youtube.com/results?sp=" + \
            filter + "&q=" + escaped_search_terms
        if self._gl:
            url = url + '&gl=' + self._gl
    ...
    def get_recommendations(self, video_id, nb_recos_wanted, depth, key):
        if video_id in self._video_infos:
            ...
            self._video_infos[video_id]['depth'] = min(
                self._video_infos[video_id]['depth'], depth)
        ...
        for video_list in soup.findAll('ul', {'class': 'video-list'}):
            if upnext:
                ...
                try:
                    recos.append(
                        video_list.contents[1].contents[1].contents[1]['href'].replace('/watch?v=', ''))
                except IndexError:
                    print (
                        'WARNING Could not get a up next recommendation because of malformed content')
                    pass
                upnext = False
            else:
                # 19 Others
                for i in range(1, 19):
                    try:
                        recos.append(
                            video_list.contents[i].contents[1].contents[1]['href'].replace('/watch?v=', ''))
                    except IndexError:
                        if self._verbose:
                            print (
                                'Could not get a recommendation because there are not enough')
                    except AttributeError:
                        if self._verbose:
                            print (
                                'WARNING Could not get a recommendation because of malformed content')
        ...
        video = self._video_infos[video_id]
        print (repr(video_id + ': ' + video['title'] + ' [' + channel + ']{' + repr(
            key) + '} ' + str(video['views']) + ' views , depth: ' + str(video['depth'])))
    ...
    def get_n_recommendations(self, seed, branching, depth, key):
        ...
        for video in self.get_recommendations(current_video, branching, depth, key):
            code = chr(index + 97)
            all_recos.extend(self.get_n_recommendations(
                video, branching, depth - 1, key + code))
            index = index + 1
        return all_recos
    ...
    def go_deeper_from(self, search_term, search_results, branching, depth):
        all_recos = self.compute_all_recommendations_from_search(
            search_term, search_results, branching, depth)
    ...
    def print_graph(self, links_per_video, only_mature_videos=True):
        ...
        for video_id in self._video_infos:
            ...
            if self.like_ratio_is_computed(video):
                ...
                popularity = video['likes'] / \
                    float(video['likes'] + video['dislikes'])
            ...
            for reco in self._video_infos[video_id]['recommendations']:
                if reco in self._video_infos:
                    links.append(
                        {'source': video_id, 'target': reco, 'value': 1})
                    link += 1
        ...
        with open('./graph-' + self._name + '-' + date + '.json', 'w') as fp:
            json.dump(graph, fp)
        print ('Wrote graph as: ' + './graph-' +
               self._name + '-' + date + '.json')
...
def compare_keywords(query, search_results, branching, depth, name, gl, language, recent, loopok):
    ...
    for keyword in query.split(','):
        yf = YoutubeFollower(verbose=True, name=keyword, alltime=False,
                             gl=gl, language=language, recent=recent, loopok=loopok)
...
def main():
    ...
    parser.add_argument('--depth', default='5', type=int,
                        help='The depth of the exploration')
    parser.add_argument('--alltime', default=False, type=bool,
                        help='If we get search results ordered by highest number of views')
    parser.add_argument(
        '--gl', help='Location passed to YouTube e.g. US, FR, GB, DE...')
    parser.add_argument(
        '--language', help='Languaged passed to HTML header, en, fr, en-US, ...')
    parser.add_argument('--recent', default=False, type=bool,
                        help='Keep only videos that have less than 1000 views')
    parser.add_argument('--loopok', default=False, type=bool,
                        help='Never loops back to the same videos.')
    ...
    compare_keywords(args.query, args.searches, args.branch, args.depth,
                     args.name, args.gl, args.language, args.recent, args.loopok)
