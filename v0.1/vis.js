function startVis(_valname,_w,_h) {
	...
	$("#visualization").html("");
	$("#visualization").css("width",(c_width+30) + "px"); //+30
...
function createInterface(_valname) {
	...
	// var _ihtml = '<label><><div uk-form-custom="target: true"><select onchange=\'colorcode(this.options[this.selectedIndex].value);\' name=\'vis_colorcoding\'>';
	var _ihtml = '<div><label>Color coding: </label>';
	_ihtml += '<div uk-form-custom="target: true"><select onchange=\'colorcode(this.options[this.selectedIndex].value);\' name=\'vis_colorcoding\'>';
	...
	_ihtml += '</select><span></span><span uk-icon="icon: chevron-down"></span></div></div>';


	_ihtml += '<div class="uk-margin-small-top"><label><input type="checkbox" class="uk-checkbox" onchange="changeInterface(\'labels\',this.checked);" id="vis_labels"/> show labels</label></div>';


			  
	if(!_singlerow) {
		_ihtml += '<div class="uk-margin-small-top"><label><input type="checkbox" class="uk-checkbox" onchange="changeInterface(\'sorting\',this.checked);" id="vis_sorting"/> Sort by value</label></div>';
		_ihtml += '<div class="uk-margin-small-top"><label><input type="checkbox" class="uk-checkbox" onchange="changeInterface(\'valign\',this.checked);" id="vis_valign"/> Align on top</label></div>';
	}
...
function canvasHihlight(d) {

	for(var _line in _lines) {
		for(var i = 0; i < _lines[_line].length; i++) {
			// _lines[_line][i].color(_lines[_line][i].concol);
			_lines[_line][i].color("#eee");
			console.log(_lines[_line][i].concol);
			if(_lines[_line][i].label) {
				_lines[_line][i].labelon = false;
				_lines[_line][i].label.remove();
...
function drawvis(_data) {
	...
	for(var _slice in _data) {
	    ...
	    if(typeof(_labels[_slice]) == "undefined") {
			...
			_labels[_slice] = r.text(_leftspacing + _barspacing * i,5,_slice.substr(5,5)).attr("text-anchor","start");
		}	
