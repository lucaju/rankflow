...
function topTenSentimentInit() {

    var windowWidth = (document.body.clientWidth*2)/3;

    var margin = {top: 30, right: 30, bottom: 30, left: 90},
        width = windowWidth - margin.left - margin.right,
        height = 180 - margin.top - margin.bottom;

    var svg = d3.select("#top-10-setiment-visualization").append("svg")
    ...
    d3.csv('data/topTenVideoSentiment.csv')
        ...
        serie.selectAll("rect")
            ...
            .attr("y", function(d) { return y(d.data.Candidate); })
            .attr("height", y.bandwidth())
            .on("mouseover",mouseOverHandle)
            .on("mouseout",mouseOutHandle)
            // .attr("width", 0)
            .transition().duration(750).delay(function(d,i) { return 1000+ (i*200);})
        ...
        g.append("g")
            ...
            .tickFormat(function(d,i) { return scale[i]; }));
        ...
        var legend = serie.append("g")
            .attr("class", "legend")
            .attr("transform", function(d,i) { return "translate(" + ((width/6)*(i+1)) + ", -30)"; });
